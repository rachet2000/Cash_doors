#This file extracts data from the control condition and puts it in the data_control dataframe. Make sure you are in the "Cash_doors" directory. 

#setwd('Cash_doors')
file_names <- list.files(path = "Control_data", pattern = "ct")
extract <- NULL
for(file in file_names){
  subject <- read.table(paste('Control_data/', file, sep = ''), header = FALSE, sep = "\t", skip = 1, blank.lines.skip = TRUE,
             fill = TRUE)
...
colnames(recall_data) <- c("recall", "door_side", "question", "door", "risky_door", "risk", "to_remove", "participant")
recall_data$to_remove <- NULL
colnames(response_data) <- c("trial", "outcome_condition", "rt", "chose_risky", "key_pressed", "reward", "color",
                             "session", "total_earned", "participant")
...
for(par in unique(extract$participant)){
  ...
  #For both sessions
    ...
    for(x in 1:nrow(temp_data)){
      ...
      data_control <- rbind(data_control, row_data)
      
    }}

data_control$condition <- factor('Control')
data_control$experiment <- factor('Control')
